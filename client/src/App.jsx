      ...
      <div className="fixed inset-0 bg-[#0c0c0e] flex items-center justify-center">
         <div className="card-surface w-full max-w-md p-10 border-white/5 bg-[#111114]/80 backdrop-blur-2xl">
            <div className="text-center mb-10">
               <div className="text-4xl font-black text-white tracking-tighter mb-2">DigitalGlide</div>
   ...
   return (
      <div className="w-[240px] bg-[var(--color-bg-sidebar)] border-r border-[var(--color-border)] h-screen flex flex-col fixed left-0 top-0 z-50">
         <div className="p-8 flex items-center gap-3">
            <div className="text-2xl font-black text-[var(--color-text-primary)] tracking-tighter cursor-pointer" onClick={() => { setView('dashboard'); setSelectedProduct(null); setSelectedBridge(null); setSelectedTableIdx(null); }}>DigitalGlide</div>
         </div>
         ...
         <div className="flex h-full w-full absolute inset-0 animate-in-slide-up bg-[#0c0c0e] z-50">
            ...
            <div className="w-[300px] border-r border-white/5 p-8 space-y-8 flex flex-col">
               <div className="flex items-center gap-4">
                  ...
                  <div><h2 className="text-lg font-black tracking-tight">{selectedBridge.name}</h2><p className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">Site Connection</p></div>
               </div>
               ...
               {selectedTableIdx !== null && bridgeProduct?.field_mapping?.tables?.[selectedTableIdx] ? (
                  <div className="space-y-8 animate-in-fade h-full flex flex-col">
                     <div className="flex justify-between items-start">
                        <div className="space-y-1">
                           ...
                           <p className="text-[10px] text-slate-500 uppercase tracking-widest">{selectedBridge.name} / Viewing Last 20 Records</p>
                        </div>
                        <button onClick={() => {
                  ...
                  <div className="h-full flex flex-col items-center justify-center opacity-30 space-y-4">
                     ...
                     <div className="text-center">
                        <h3 className="text-xl font-black tracking-tight">Site Data Viewer</h3>
                        <p className="text-[10px] font-bold uppercase tracking-widest mt-1">Select a database table to view real-time data from this connection.</p>
                     </div>
                  </div>
               )}
            </div>
         ...
         <div className="flex-1 ml-[240px] relative">
            <header className="h-[80px] border-b border-white/5 flex items-center justify-between px-10 sticky top-0 bg-[#0c0c0e]/80 backdrop-blur-xl z-40">
               <div className="flex items-center gap-4">
                  ...
                  <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest leading-none">Admin / System Monitoring</div>
               </div>
               ...
               <div className="p-8 space-y-8 animate-in-fade">
                  ...
                  <div className="flex items-center gap-3">
                     <div className="relative group">
                        ...
                        <input placeholder="Search Connections..." className="bg-white/[0.02] border border-white/5 rounded-xl pl-10 pr-6 py-2.5 text-xs outline-none focus:bg-white/[0.04] focus:border-white/10 transition-all w-[300px]" />
                     </div>
                  </div>
                                 ...
                                 <td className="px-6 py-6 font-bold">
                                    <div className="flex items-center gap-4">
                                       ...
                                       <div className="flex flex-col">
                                          ...
                                          <span className="text-[10px] text-slate-500 font-bold uppercase tracking-tighter opacity-60 mr-1">{b.stats?.license || 'Scanning Network...'}</span>
                                       </div>
                                    </div>
                                 </td>
                                 ...
                                 <td className="px-6 py-6 font-medium">
                                    <div className="flex flex-wrap gap-1.5">
                                       {(b.tags || '').split(',').filter(t => t.trim()).map((tag, tIdx) => (
                                          <span key={tIdx} className="px-2 py-0.5 bg-indigo-500/10 border border-indigo-500/20 rounded text-[7px] font-black uppercase text-indigo-400 tracking-widest">{tag.trim()}</span>
                                       ))}
                                       {(!b.tags || b.tags.trim() === '') && <span className="text-[10px] text-slate-700 italic opacity-20">—</span>}
                                    </div>
                                 </td>
                                 <td className="px-6 py-6">
                                    <div className={`inline-flex items-center gap-1.5 px-3 py-1 border rounded-full text-[9px] font-black uppercase tracking-widest ${b.status === 'Online'
                                    ...
                                    <div className="flex justify-end items-center gap-2 opacity-0 group-hover:opacity-100 transition-all">
                                       <button onClick={(e) => { e.stopPropagation(); fetchLogs(b.id); }} className="p-2 text-slate-700 hover:text-white hover:bg-white/5 rounded-lg transition-all" title="View Logs"><Eye size={16} /></button>
                                        {products.find(p => String(p.id) === String(b.product_id))?.field_mapping?.lock_config && (
                                           <button onClick={(e) => { e.stopPropagation(); toggleLock(b.id); }} className={`p-2 rounded-lg transition-all ${b.stats?.license === 'Locked' ? 'text-amber-500 hover:bg-amber-500/10' : 'text-slate-700 hover:text-emerald-500 hover:bg-white/5'}`} title={b.stats?.license === 'Locked' ? 'Unlock System' : 'Lock System'}>
                                       ...
                                       <button onClick={(e) => {
                                          ...
                                          setShowBridgeModal(true);
                                       }} className="p-2 text-slate-700 hover:text-indigo-400 hover:bg-white/5 rounded-lg transition-all" title="Edit Site"><Pencil size={16} /></button>
                                       <button onClick={(e) => { e.stopPropagation(); deleteBridge(b.id); }} className="p-2 text-slate-700 hover:text-rose-500 hover:bg-white/5 rounded-lg transition-all" title="Remove Connection"><Trash2 size={16} /></button>
                                    </div>
                                 </td>
                              </tr>
                           ))}
                        ...
                        <tbody className="divide-y divide-white/[0.03]">
                           {products.map(p => (
                              <tr key={p.id} className="cursor-pointer hover:bg-white/[0.02] transition-colors group" onClick={() => handleEditProduct(p)}>
                                 ...
                                 <td className="px-6 py-6 font-mono text-[10px] text-slate-500">
                                    {bridges.filter(b => b.client_id === p.id).length} Connections
                                 </td>
