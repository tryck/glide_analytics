...
import { Moon, Sun, RefreshCw, Calendar, LayoutGrid, List } from 'lucide-react';
      ...
      <header className="h-[60px] border-b border-[var(--color-border)] flex items-center justify-between px-10 sticky top-0 bg-[var(--color-bg-header)] backdrop-blur-xl z-[60] fixed right-0 left-[230px] shadow-[var(--color-topbar-shadow)]">
         <div className="min-w-0 flex items-center gap-3">
            <div className="leading-tight min-w-0">
               <h1 className="text-[18px] font-black text-[var(--color-text-strong)] tracking-tight truncate">{title}</h1>
         ...
         <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-2 p-1 rounded-lg border border-[var(--color-border)] bg-[var(--color-panel-soft)]">
               <div className="flex items-center gap-1 border-r border-[var(--color-border)] pr-1 mr-1">
                  <button
                     onClick={() => setViewMode && setViewMode('grid')}
                     className={`p-2 rounded-md transition-all ${viewMode === 'grid' ? 'bg-[var(--color-accent)] text-white shadow-lg' : 'text-[var(--color-text-muted)] hover:text-[var(--color-text-primary)]'}`}
                  ...
                  <button
                     ...
                     className={`p-2 rounded-md transition-all ${viewMode === 'table' ? 'bg-[var(--color-accent)] text-white shadow-lg' : 'text-[var(--color-text-muted)] hover:text-[var(--color-text-primary)]'}`}
