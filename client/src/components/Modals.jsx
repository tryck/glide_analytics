        ...
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-6 backdrop-blur-2xl bg-black/60">
            <div className="card-surface w-full max-w-5xl h-[80vh] p-10 relative flex flex-col" style={{ borderRadius: 'var(--radius-sm)' }}>
                <div className="flex justify-between items-start mb-8">
                    <div><h2 className="text-3xl font-black tracking-tighter">Audit Control Center</h2><p className="text-[10px] text-slate-500 tracking-widest mt-1">Remote Access / Last 100 System Activities</p></div>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar border border-white/5 bg-black/40" style={{ borderRadius: 'var(--radius-sm)' }}>
                    {logLoading ? (
                        <div className="h-full flex flex-col items-center justify-center space-y-4 opacity-50">
                            ...
                            <div className="text-[10px] font-black tracking-[0.2em]">Connecting to Site...</div>
                        </div>
                    ) : (
                        <table className="w-full text-left border-collapse">
                             <thead className="sticky top-0 bg-[#161619] text-[9px] font-black text-slate-600 tracking-widest border-b border-white/5 z-10">
                                <tr>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 backdrop-blur-xl bg-black/40">
            <div className="card-surface w-full max-w-2xl p-10 relative space-y-8" style={{ borderRadius: 'var(--radius-sm)' }}>
                <div className="flex justify-between items-start">
                    <div><h2 className="text-2xl font-black tracking-tighter">{bridgeForm.id ? 'Edit Site Connection' : 'New Site Connection'}</h2><p className="text-[10px] text-slate-500 tracking-widest mt-1">Setting up system link</p></div>
                ...
                <form onSubmit={handleSaveBridge} className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-6">
                        <div className="space-y-2">
                            <label className="text-[10px] font-bold text-slate-500 tracking-widest px-1">Database Name</label>
                        ...
                        <div className="space-y-2">
                            <label className="text-[10px] font-bold text-slate-500 tracking-widest px-1">Tags (Comma Separated)</label>
                    ...
                    <div className="space-y-2">
                        <label className="text-[10px] font-bold text-indigo-500 tracking-widest px-1">Connection Key</label>
                    ...
                    <button type="submit" className="w-full bg-indigo-600 hover:bg-indigo-500 py-4 text-[11px] font-black tracking-widest transition-all" style={{ borderRadius: 'var(--radius-sm)' }}>
            ...
            <div className="card-surface w-full max-w-md p-10 relative space-y-8 animate-in-slide-up border-white/10 shadow-[0_0_100px_rgba(99,102,241,0.15)] bg-[#111114]/90" style={{ borderRadius: 'var(--radius-sm)' }}>
                <div className="flex justify-between items-start">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-2xl font-black tracking-tighter text-white">{clientForm.id ? 'Modify Client' : 'Register Client'}</h2>
                            <p className="text-[10px] text-slate-500 tracking-widest mt-1">{clientForm.id ? 'Update existing entity' : 'Initialize New Managed Entity'}</p>
                        </div>
                    </div>
                ...
                <form onSubmit={async (e) => {
                    ...
                    if (clientForm.name) {
                        try {
                            if (clientForm.id) {
                                await axios.put(`${API_BASE}/clients/${clientForm.id}`, clientForm);
                            } else {
                                await axios.post(`${API_BASE}/clients`, clientForm);
                            }
                            setShowClientModal(false);
                            fetchData();
                        } catch(e) { alert("Failed to save client: " + (e.response?.data?.error || e.message)); }
                    }
                }} className="space-y-6 pt-4">
                    <div className="space-y-2">
                        <label className="text-[10px] font-bold text-slate-500 tracking-widest px-1">Legal Entity Name</label>
                        <input 
                            autoFocus 
                            required 
                    ...
                    <div className="flex gap-3 pt-4">
                        <button type="button" onClick={() => setShowClientModal(false)} className="flex-1 bg-white/[0.03] hover:bg-white/[0.06] border border-white/5 py-4 text-[11px] font-black tracking-widest transition-all text-slate-400" style={{ borderRadius: 'var(--radius-sm)' }}>Cancel</button>
                        <button type="submit" className="flex-1 bg-indigo-600 hover:bg-indigo-500 py-4 text-[11px] font-black tracking-widest transition-all shadow-[0_10px_30px_rgba(79,70,229,0.3)] text-white" style={{ borderRadius: 'var(--radius-sm)' }}>
                            {clientForm.id ? 'Save Changes' : 'Register Site'}
                        </button>
                    </div>
                </form>
            </div>
